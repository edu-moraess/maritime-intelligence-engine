...
from src.ml.temporal import (
    ...
    MINIMUM_TRACKS_FOR_DEEP_MODEL,
    TemporalAnomalyAdapter,
    TemporalTrainer,
    TrainingConfig,
...
def test_not_ready():
    r = TemporalAnomalyAdapter().fit(
        {f"3682076{i:02d}": _track(f"3682076{i:02d}", 6, 25 + i * 0.01) for i in range(3)}
    )
    assert r.status == "NOT_READY"
    assert r.scores == []
    assert r.n_tracks_usable == 3


@pytest.mark.skipif(not torch_available(), reason="no torch")
def test_adapter_ready():
    tracks = {
        f"3682076{i:02d}": _track(f"3682076{i:02d}", 6, 25 + i * 0.02)
        for i in range(MINIMUM_TRACKS_FOR_DEEP_MODEL)
    }
    r = TemporalAnomalyAdapter().fit(tracks)
    assert r.status == "READY"
    assert r.training_completed and r.inference_available
    assert r.model_state is not None
    assert r.scaler_mean is not None and r.scaler_scale is not None
...
def test_engine_ready():
    ...
    assert s.readiness.temporal_status == "READY"
    prev = e.temporal
    e._recompute()
    assert e.temporal is prev
...
def test_clear():
    e = MaritimeIntelligenceEngine(AppSettings(aisstream_api_key="k", bbox=DEFAULT_BBOX))
    e.store.extend(_track("368207620", 5))
    e._recompute()
    e.clear_session_data()
...
def test_unavailable(monkeypatch):
    ...
    r = TemporalAnomalyAdapter().fit(
        {f"3682076{i:02d}": _track(f"3682076{i:02d}", 5) for i in range(8)}
    )
    assert r.status == "UNAVAILABLE"


def test_temporal_failure_does_not_break_classical(monkeypatch):
    """Deep Temporal isolation: classical embeddings still run if temporal fails."""
    e = MaritimeIntelligenceEngine(AppSettings(aisstream_api_key="k", bbox=DEFAULT_BBOX))
    rows = []
    for v in range(5):
        rows.extend(_track(f"3682076{v:02d}", 6, 25 + v * 0.02))
    ...
    monkeypatch.setattr(e.temporal_adapter, "fit", boom)
    e._recompute()
    assert e.temporal is not None
    assert e.temporal.status == "FAILED"
...
def test_trainer_direct():
    ...
    assert len(seqs) >= MINIMUM_TRACKS_FOR_DEEP_MODEL
    tr = TemporalTrainer(TrainingConfig(max_training_seconds=3.0, seed=42)).train(seqs)
    assert tr.ok
    assert tr.model_state is not None
    assert tr.training_completed
    assert tr.best_loss is not None and tr.best_loss >= 0


def test_benchmark_inconclusivo_sparse():
    from src.ml.temporal.types import TemporalScore

    r = compare_if_vs_deep(
        {"368207620": 0.9},
        [TemporalScore("368207620", 0.1, 0.9)],
    )
