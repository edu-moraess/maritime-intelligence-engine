...
def observations_frame(observations: Iterable[AISObservation]) -> pd.DataFrame:
    ...
    if not rows:
        return pd.DataFrame(columns=["mmsi", "timestamp", "latitude", "longitude", "sog_knots", "cog_degrees", "vessel_name"])
...
def hourly_volume(observations: Iterable[AISObservation]) -> pd.DataFrame:
    ...
    return frame.groupby("hour", as_index=False).agg(messages=("mmsi", "size"))
