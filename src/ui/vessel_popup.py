...
def render_vessel_quick_intelligence(vessel, snapshot, *, show_gemini_hook=True):
    ...
    cols = st.columns(4)
    cols[0].markdown(_kv("SOG", f"{float(sog):.1f} kn" if sog is not None else "—"), unsafe_allow_html=True)
    cols[1].markdown(_kv("COG", f"{float(cog):.0f}°" if cog is not None else "—"), unsafe_allow_html=True)
    cols[2].markdown(_kv("HDG", f"{float(hdg):.0f}°" if hdg is not None else "—"), unsafe_allow_html=True)
    cols[3].markdown(_kv("POS", f"{float(lat):.3f}, {float(lon):.3f}" if lat is not None and lon is not None else "—"), unsafe_allow_html=True)
