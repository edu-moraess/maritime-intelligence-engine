...
Combines Vessel Intelligence Profile v1 + Historical + Behavioral Intelligence
with Operations Frontend presentation (expanders, provenance badges, LIVE line).
"""
...
def render_vessel_quick_intelligence(
    ...
    *,
    ...
    if telemetry.cog_degrees is not None:
        telem_parts.append(f"{telemetry.cog_degrees:.0f}°")
    ...
    # Identity (always visible — compact)
    section_kicker("Identity")
    metric_strip(
        {
            "MMSI": identity.mmsi,
    ...
    if not telemetry.available:
        ...
        if telemetry.latitude is not None and telemetry.longitude is not None:
            st.markdown(
                f"<div class='small-note' style='margin:.15rem 0 .35rem'>"
    ...
    with st.expander(
        "Historical profile",
        expanded=bool(historical.available and historical.status != "N/A"),
    ):
        st.markdown(provenance_badge("HISTORICAL"), unsafe_allow_html=True)
    ...
    # Deterministic Behavioral Intelligence (Etapa 4)
    ...
    with st.expander("Behavioral signals", expanded=False):
        ...
        if (
            ...
            and mmsi in getattr(embedding, "mmsis", [])
        ):
            ...
            st.markdown(
                "<div class='small-note'>Session-relative ranking — "
                "not a calibrated probability.</div>",
                unsafe_allow_html=True,
            )
        else:
            st.markdown(provenance_badge("INSUFFICIENT DATA"), unsafe_allow_html=True)
    ...
    with st.expander(
        "Anomaly assessment",
        expanded=bool(anomalies.current_session),
    ):
        ...
        if photo:
            ...
            try:
                ...
                if photo:
                    ...
                    notice(
                        "No verified vessel image was found for this MMSI.",
                        "yellow",
                    )
            except Exception:
                notice(
