...
from src.ui.presentation import empty_state, metric_strip, notice, panel_title
...
def render_similarity(engine: MaritimeIntelligenceEngine, snapshot: EngineSnapshot, settings: AppSettings) -> None:
    ...
    if len(track) < 2:
        empty_state("Similarity search requires at least 2 real AIS position reports for the reference vessel. "
...
def render_anomalies(engine: MaritimeIntelligenceEngine, snapshot: EngineSnapshot, settings: AppSettings) -> None:
    ...
    metric_strip({"FINDINGS": len(findings), "VESSELS": len({f.mmsi for f in findings}),
                  "HIGH SCORE": f"{max((f.score for f in findings), default=0):.2f}"})
    if not findings:
        empty_state("Anomalies are calculated only from currently observed real AIS reports. No finding is fabricated when data is insufficient. "
                    "Collect real AIS data for longer or select a denser monitoring region if more history is required.",
    ...
    table = pd.DataFrame([{
        "Timestamp": _utc(f.received_at), "MMSI": f.mmsi,
        "Location": f"{f.latitude:.4f}, {f.longitude:.4f}", "Score": f"{f.score:.2f}",
        "Category": f.category, "Confidence": f"{f.confidence:.2f}" if f.confidence is not None else "N/A",
...
def render_traffic(engine: MaritimeIntelligenceEngine, snapshot: EngineSnapshot, settings: AppSettings) -> None:
    ...
    metric_strip({"ACTIVE VESSELS": summary["active_vessels"], "MESSAGES": f"{summary['messages']:,}",
                  "AVG SPEED": f"{summary['average_speed_knots']:.1f} kn", "REGIONS": summary["regions"],
...
def render_data_quality(engine: MaritimeIntelligenceEngine, snapshot: EngineSnapshot, settings: AppSettings) -> None:
    ...
    metric_strip({"QUALITY": f"{report.quality_percent:.1f}%", "MESSAGES": f"{report.messages_processed:,}",
                  "INVALID": f"{report.invalid_records:,}", "DUPLICATES": f"{report.duplicate_records:,}",
                  "LAST UPDATE": _utc(snapshot.status.last_received_at)})
    st.write("")
    left, right = st.columns([1.2, 1], gap="medium")
    with left:
        rows = pd.DataFrame({"Check": ["Invalid coordinates / records", "Duplicate messages", "Missing values", "Timestamp gaps",
                                         "Invalid MMSI", "Impossible speeds", "Impossible geographic jumps", "Stale reports"],
                             "Count": [report.invalid_records, report.duplicate_records, report.missing_values, report.receive_time_gaps,
                                       report.invalid_mmsi, report.impossible_speeds, report.impossible_jumps, report.stale_records]})
...
def render_system(engine: MaritimeIntelligenceEngine, snapshot: EngineSnapshot, settings: AppSettings) -> None:
    ...
    with left:
        ...
        if len(regions) > 1:
            st.write("**Monitoring areas:**")
            for index, bbox in enumerate(regions[:2]):
                st.write(f"**Region {chr(65 + index)}:** `{bbox[0]} → {bbox[1]}`")
        elif regions:
            st.write(f"**Monitoring area:** `{regions[0][0]} → {regions[0][1]}`")
        else:
    ...
    if temporal is None:
        ...
        metric_strip({"STATUS": temporal.status, "MODEL": temporal.method, "TRACKS": f"{temporal.n_tracks_usable}/{temporal.n_tracks_seen}",
                      "TRAINING": f"{epochs} ep · {train_s}", "LOSS": loss,
        ...
        if temporal.scores:
            ...
            lines = [f"`{s.mmsi}`  recon={s.reconstruction_error:.6f}  deep={s.deep_anomaly_score:.3f}" for s in top]
            st.markdown("**Top deep scores (session-relative ranking):**\n\n" + "\n\n".join(lines))
        if temporal.status == "READY":
            notice("Deep Temporal trained on real AIS only. deep_anomaly_score is session-relative ranking, not probability.", "green")
