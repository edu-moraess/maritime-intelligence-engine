...
STATUS_HALO = {"NORMAL":[53,194,201,0],"ATTENTION":[233,184,87,0],"ANOMALY":[239,107,115,45],"CRITICAL":[220,50,60,60],"SELECTED":[255,255,255,75],"STALE":[121,147,155,0]}
STATUS_RING = {"NORMAL":[53,194,201,0],"ATTENTION":[233,184,87,0],"ANOMALY":[239,107,115,150],"CRITICAL":[220,50,60,180],"SELECTED":[255,255,255,220],"STALE":[121,147,155,0]}
...
def resolve_course_degrees(row): return resolve_vessel_course(row.get("heading_degrees"), row.get("cog_degrees"))
...
def build_track_segments(observations, *, selected):
    ...
    def _ts(obs):
        ...
        if isinstance(ts, datetime): return ts if ts.tzinfo else ts.replace(tzinfo=timezone.utc)
...
def operational_strip(*, live_state, targets, tracks, anomalies, region):
    ...
    return (f"<div style='display:flex;flex-wrap:wrap;gap:1.1rem;align-items:center;font-family:IBM Plex Mono,monospace;font-size:0.68rem;letter-spacing:.07em;margin:0 0 .4rem;color:#79939b'>" f"<span style='color:{state_color};font-weight:600'>{live_state}</span>" f"<span>TARGETS {targets}</span><span>TRACKS {tracks}</span>" f"<span>ANOMALIES {anomalies}</span><span>REGION {region}</span></div>")
