...
def density_points_from_observations(observations, *, max_points=2500):
    """Build real AIS position points for optional density rendering.

    Returns only coordinates derived from observations. Never synthesizes traffic.
    """
    points = []
...
def build_density_layer_spec(points: list[dict]) -> dict | None:
    ...
    if not points:
        return None
    return {
        "type": DENSITY_LAYER_TYPE,
        "data": points,
        "get_position": ["longitude", "latitude"],
        "get_fill_color": list(DENSITY_FILL_COLOR),
        "get_radius": DENSITY_RADIUS,
        "radius_min_pixels": DENSITY_RADIUS_MIN_PX,
        "radius_max_pixels": DENSITY_RADIUS_MAX_PX,
        "pickable": False,
    }
