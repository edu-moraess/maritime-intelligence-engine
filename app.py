...
def _with_bbox(settings: AppSettings, bbox: tuple[tuple[float, float], tuple[float, float]], config_error: str | None = None) -> AppSettings:
    return AppSettings(
...
def _render_sidebar(settings: AppSettings) -> tuple[AppSettings, str, bool, bool]:
    with st.sidebar:
        ...
        with st.expander("Monitoring region", expanded=False):
            min_lat = st.number_input("Min latitude", value=float(settings.bbox[0][0]), min_value=-90.0, max_value=90.0, step=0.01, format="%.5f", key="bbox_min_lat")
            min_lon = st.number_input("Min longitude", value=float(settings.bbox[0][1]), min_value=-180.0, max_value=180.0, step=0.01, format="%.5f", key="bbox_min_lon")
            max_lat = st.number_input("Max latitude", value=float(settings.bbox[1][0]), min_value=-90.0, max_value=90.0, step=0.01, format="%.5f", key="bbox_max_lat")
            max_lon = st.number_input("Max longitude", value=float(settings.bbox[1][1]), min_value=-180.0, max_value=180.0, step=0.01, format="%.5f", key="bbox_max_lon")
            ...
            if region_error is None and candidate_bbox != settings.bbox:
                ...
                settings = _with_bbox(settings, settings.bbox, region_error)
        st.markdown("<hr>", unsafe_allow_html=True)
...
def main() -> None:
    ...
    if snapshot.status.state != "LIVE AIS":
        notice(f"{snapshot.status.state}: {snapshot.status.reason}", "red" if snapshot.status.state in {"DISCONNECTED", "REAL AIS DATA UNAVAILABLE"} else "")
    if page == "Overview":
