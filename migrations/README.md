...
`003_fix_observation_conflict_target.sql` adds the exact composite unique index required by the writer's `ON CONFLICT (session_id, payload_hash)` target. `004_harden_historical_schema.sql` is an idempotent reconciliation migration: it reasserts PostGIS availability, the composite conflict index, the normal AIS-second constraint, and the seeded region definitions without overwriting operator data. This protects deployments where the database was partially provisioned or migrated outside the MIE ledger.
